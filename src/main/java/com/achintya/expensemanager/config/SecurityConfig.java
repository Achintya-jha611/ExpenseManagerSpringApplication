...
public class SecurityConfig {
    ...
    public SecurityFilterChain enableSecurity(HttpSecurity httpSecurity){
        return httpSecurity.authorizeHttpRequests((authorizeHttpRequests)->
                authorizeHttpRequests.requestMatchers("/users/**","/user").authenticated().requestMatchers("/expenses/**").permitAll().anyRequest().authenticated()).httpBasic(Customizer.withDefaults())
